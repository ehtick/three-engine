...
const OUT = ".gi-shots/normal-weight";
...
async function boot(label, farOff) {
  ...
  await page.evaluateOnNewDocument((project, off) => {
    ...
    if (!off) globalThis.__giGatherNormalWeight = true;
  }, PROJECT, farOff);
  console.log(`[${label}] opening … (${farOff ? "normal weight OFF (today default)" : "normal weight ON"})`);
  ...
  for (let i = 0; i < 700 && !built; i++) await wait(1000);
...
const off = { mean: 99.5, std: 53.2, p5: 10.6, p95: 189.4, under: 65.3 }; // measured baseline, prior run of THIS rig/pose/settle
if (!on || !off) process.exit(1);
console.log("");
console.log(`contrast (std)   normal-weight ON ${on.std.toFixed(1)}  vs OFF ${off.std.toFixed(1)}   (${(((on.std-off.std)/off.std)*100).toFixed(0)}%)`);
console.log(`darks    (p5)    normal-weight ON ${on.p5.toFixed(1)}  vs OFF ${off.p5.toFixed(1)}   (lifted ${(on.p5-off.p5).toFixed(1)})`);
console.log(`under awnings    normal-weight ON ${on.under.toFixed(1)}  vs OFF ${off.under.toFixed(1)}`);
