...
Pass another manifest URL to `loadWorldSurfaceMaps`. Its `assets` array should contain exactly one entry for each `role`: `grass`, `soil` and `rock`. Each entry needs:
