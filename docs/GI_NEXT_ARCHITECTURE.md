...
*Written 2026-08-07, after the block-size sweep. Supersedes the "voxelSize sets the block size"
hypothesis in the session-34/35 handoffs — **that hypothesis is now falsified by measurement**
(§1). This document is the plan; `GI_PLAN.md`, `GI_SHADOWS_PLAN.md` and
`dynamic_gi_exact_dynamic_objects.md` remain the history of what got us here.*
...
1. **The blockiness is not the voxels.** A 5× sweep of `voxelSize` moves the measured block size
   ...
   artifact; the c0 probe lattice and the cascade merge are not.
2. **Two thirds of the floor structure was one `floor()`** in the parallax merge — already found
   and already fixed in the working tree. Phase 0 is to verify and land it.
...
- **Any fix aimed at the voxel cell will not move the block size.** Do not spend a session on
  ...
  shrink.
- **Block size is sub-proportional to probe spacing** — roughly `0.19 + 0.17·spacing` in x. The
  0.19 m intercept is a *floor* that does not follow either dial. That floor is a separate term
...
**Flicker is not in this rig.** Every arm has `adopted: 0` and a static panel, and a settled frame
is deterministic to 1e-4. The user's flicker complaint is about a *rotating* mover and about
*emissive* meshes — `MOVER=1` exists in the rig and has not been run. **That is unmeasured, and
Phase 0 owes it a number.**

### 1.2 The parallax merge's `floor()` — found, fixed, unverified
...
it. Ablation on the block rig:

- baseline (snap on): modulation **14.71%**
- `__giParallaxMerge = false` (correction off entirely): **4.58%**

So **two thirds of all the structure on that floor was that one `floor()`**. The working tree
already carries the continuous form — a separable 3-tap `[(1−f)/2, ½, f/2]` box per axis, same
box *width* (dilution preserved), fractional *centre*; 9 reads instead of 4, in a loop already
reading 8 parent probes. Hatch `__giParallaxMergeSmooth = false` restores the snap.

**The number that does not exist yet: what the SMOOTH form measures.** Somewhere in [4.58%,
14.71%]. Phase 0 exists to produce it.
...
```
  ...
  TRANSPORT            │ radiance cascades, c0…cN                 │  unchanged shape
                       │   merge: continuous parallax (Phase 0)   │
...
**Nothing new is designed until these numbers exist.** The working tree carries uncommitted work
from sessions 33–36 across `GISystem.js` (+704), `dynamicObjects.js` (+196), `giField.js` (+116),
`cascadeMerge.js` (+69), `giLight.js` (+37), `cascadeGather.js` (+22) plus 11 new probe scripts.
It also carries **unrelated user WIP** in `src/editor`, `mcp/`, `src-tauri`, `scripts/run-*-test.mjs`
— **ask before sweeping any of that into a commit.**

**0.1 — The smooth-parallax number.**
```
node node_modules/vite/bin/vite.js --port 5201 --strictPort     # the user's server; do not start a second
ABLATE="|__giParallaxMergeSmooth=false|__giParallaxMerge=false" node scripts/run-gi-block-size.mjs
```
Expect three modulation numbers. Baseline (smooth, default) must be materially below 14.71%.
**Gate: smooth ≤ 8%.** If it lands near 14.7% the fix is not doing what the comment claims and
Phase 1 starts by finding out why, not by adding a second fix on top.

**0.2 — Re-run the probe sweep with the fix on.** Same command as §1.1. The question is whether
`blockX` still tracks `probeSpacing` and what the intercept is now. Three outcomes, three
different Phase 1s:
- intercept collapses and the slope survives → the probe lattice is the whole remaining story →
  Phase 1 is the reconstruction/probe work.
- intercept survives → there is a fixed-width term downstream (resolve reconstruction or the
  bilateral) → Phase 1 is a screen-side ablation.
- both collapse → skip Phase 1 entirely, go to Phase 2.

**0.3 — The flicker number that has never been taken.** `MOVER=1` puts the panel on
`giMobility: "dynamic"` — the user's rotating-box case, an exact mover with no voxels of its own.
Run the sweep with `MOVER=1` and read `flick.rms` and `flick.halfX`. This is the only measurement
that speaks to "jumpy", as opposed to "boxy". Extend the rig to *rotate* the mover if the static
mover shows nothing — `run-gi-mover-bounce.mjs` and the `SPIN=1` arm in the real-shadow probe
already have the pattern.

**0.4 — Emissive arm.** `MODE=emissive` is the rig default; also run `MODE=albedo MOVER=1`. The
user's claim is that emissive blockiness and mover-bleed blockiness are *the same artifact*.
After 0.1 that claim is re-testable: if both drop by the same factor, they were both the merge.

**0.5 — Commit.** GI-module changes only, in reviewable slices, with the suite list from the
session-35 handoff green: `check:types`, `gi-gpu-smoke storage=8` on `?dynobj=2` and
`?mode=hybrid-exact-complex&dynobj=2`, `test:gi-sunleak`, `test:gi-lightvis`, `test:gi-dynobj`,
`test:gi-occupancy`, `test:gi-spawn`. Add the new probes to `package.json` (`probe:gi-block-size`
is already wired) and **check the standing rule**: any new tunable must be reachable over MCP in
the same change, not later.

**Exit:** the tree is committed, and §1.1's table has a post-fix twin.

---

## 5. Phase 1 — the probe-lattice residual

*Scope decided by Phase 0.2. Do not pre-commit to a fix.*

The candidates, in the order the evidence would pick them:

**1a. The reconstruction floor (if the 0.19 m intercept survives).** Ablate the screen side with
the block rig: resolve scale, the bilateral filter radius, the checker/rate cadence
...
under a `__gi…` hatch like everything else in this module.
...
**Exit gate:** modulation ≤ 4% on the emissive arm and ≤ 6% on the `MOVER=1` arm, or a written
argument that the remaining structure is *not* lattice-shaped and therefore belongs to Phase 2.
...
is a much larger change and it should not be sold as a blockiness fix if the merge already was
one.
...
### 6.5 Why the binding wall does not block this

The module's standing rule is the **portable 8-storage-buffer stage limit**, and the composed
cascade/shadow kernels sit exactly at it (`dynamicObjects.js:38`, `occupancyField.js:193`).
Resolve and composite sit at the **12-uniform-buffer** wall.

Sampled textures are a **different binding class**. The WebGPU baseline is 16 sampled textures and
16 samplers per stage, and the trace kernels currently read **one** 3D texture (`distanceTexture`,
plus the optional sparse page/pool). Adding `srcRadiance` costs **1 of 16**, not 1 of 8.

That is the fact that makes Phase 2 possible now and did not when the mean-albedo comment was
written. **Verify it on the smoke harness before building anything else:** `gi-gpu-smoke` must
still report `storage=8` on every arm.
...
- **`probe:gi-block-size` with `MOVER=1`** — the block size at a mover hit must now be the card
  texel, i.e. below the measurement floor.
- **`gi-gpu-smoke`** — new arm `?srccache=1`; `storage=8` on every arm, non-negotiable.
...
- `storage` bindings exceed 8 on any smoke arm and cannot be brought back.
...
|---|---|---|---|
| 0 | verify + commit the tree; the four missing numbers | — | 1 session |
| 1 | probe-lattice residual (scope from 0.2) | 0 | 1–2 sessions |
...
**Start at 0.1.** Everything else in this document is conditional on what it returns.
